...
class ChangeRequest(models.Model):
    ...
    # Point directly to your StudentProfile model class
    student = models.ForeignKey(StudentProfile, on_delete=models.CASCADE, related_name="requests")
    message = models.TextField()
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='pending')
    created_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"Request from {self.student.user.email} - {self.status}"